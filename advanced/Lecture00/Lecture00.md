...
比如char转换为short，在多处的八字节默认是补1的。
如果是unsigned char，则补0.

大端小端法是对于多字节数据而言的，如果是一个char数组，则各个char数据是按内存地址从低到高排列的。
在实现bitset时会遇到大端小端的问题，以及还有一个字节内部的各bit存储的问题。
GPU中titan XP也是采用小端法存储的，这些都是可以检验的。
...
int的最小值可用INT_MIN，是负值。
但浮点数的最小值有所不同，DBL_MIN是指最小正数，-DBL_MAX才是最小负数。
浮点数的临界值定义在floats.h中，而其他类型的最值定义在limits.h头文件中。
...
一个核虽然支持超线程技术，但如果两个线程真是满负荷工作，效率可能比串行执行还低。
比较好的方式是一个线程负荷高，一个线程负荷低。
或者两个线程各自有一些耗时的访存操作，穿插执行。
