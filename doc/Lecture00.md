...
软链接可以为目录设置，硬链接则不能为目录设置。
软链接可以不设置为绝对路径，可以就是个文件名(这样默认在当前目录查找)，也可以指定为绝对路径，但软链接本身的大小就是其所指向路径的字符个数。
（无论是绝对路径还是相对路径）
删除软链接可以直接用rm命令，rm xxx.sln，不会删除指向的文件

用户a无法读写另一个用户的主目录，除非利用管理员权限。
root账户下将用户a的文件拷贝到用户b的主目录下，那么b的目录下该文件的所有者将是root。
如果是mv操作，所有者将是a。
用`sudo`命令也是同理。
...
管理员权限下使用 passwd -d xxx 清空 xxx 用户的密码
...
~/.ssh 的权限应是700，其下面config文件和authorized_keys的权限应是600 （新建文件的默认权限是644）

匿名用户，对于有些应用比如nfs可能还有nfsnobody
